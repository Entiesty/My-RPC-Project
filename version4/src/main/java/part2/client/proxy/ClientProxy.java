...
public class ClientProxy implements InvocationHandler {
    ...
    public ClientProxy() {
        ...
        this.client = new NettyRpcClient(serviceCenter);
    ...
    public Object invoke(Object proxy, Method method, Object[] args) {
        try {
            ...
            if (serviceCenter.checkRetry(request.getInterfaceName())) {
                response = new GuavaRetry().sendRequestWithRetry(request, client);
            } else {
...
/*            int i = Integer.parseInt(Thread.currentThread().getName().substring(6));
            if (i % 19 == 0) {
                circuitBreaker.recordFailure();
            } else {
                circuitBreaker.recordSuccess();
            ...
            logger.info("服务器返回的代码为{}", response.getCode());
